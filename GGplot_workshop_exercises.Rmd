...
`?geom_point()`Under Aesthetics we can see that this layer understands the following aesthetics:
 x
 y
 alpha
 colour
...
3) Make the plots easier to read by adjusting the Y scale in each row (check `?facet_grid` to find the correct option) facet_grid draws 1 plot on a canvas. We can draw different plots on 1 canvas using grid.arrange (libraries grid & gridExtra)
Shape of grid can be specified by a `layout_matrix`
We need to pass a list of ggplot objects (or 'grobs') as input
*A graphical object (“grob”) is a description of a graphical item. These basic classes provide default behavior for validating, drawing, and modifying graphical objects.Exercises:
