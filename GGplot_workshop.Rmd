...
This is an R-markdown (`.Rmd`) file.
It should be opened with RStudio.
...
A cell can have one or more outputs, including text, tables and plots.
You can navigate outputs below the cell using the displayed arrows.
...
After all, we did tell ggplot to map Petal.Length to the x axis and Petal.Width to the y axis?
...
# ALERT! ACHTUNG! DANGER! PERICOLO!
# x and y from ggplot() function are still being mapped in the layer geom_rect(). Meaning you are drowing 1 rectangle for each x+y from ggplot() (alpha not working as expected due to overlapping rectangles) 

```
```{r}
# Solutions
# Lowering value for alpha even more 
ggplot(iris, aes(x=Petal.Length, y=Petal.Width, color=Species)) +
  geom_point(size=4, shape=3) +
  geom_smooth(formula = y ~ x, method='lm') +
  geom_rect(aes(xmin=0.9, xmax=2, ymin=0, ymax=0.75), alpha=0.002, fill="green", color=NA) 

# Use annotate() instead of geom_rect(). annotate() does not inherit the mapping from ggplot()
ggplot(iris, aes(x=Petal.Length, y=Petal.Width, color=Species)) +
  geom_point(size=4, shape=3) +
  geom_smooth(formula = y ~ x, method='lm') +
  annotate(geom="rect", xmin=0.9, xmax=2, ymin=0, ymax=0.75, alpha=.2, color=NA, fill="yellow")

# or (silly bypass) use new 'data' for geom_rect()
ggplot(iris, aes(x=Petal.Length, y=Petal.Width, color=Species)) +
  geom_point(size=4, shape=3) +
  geom_smooth(formula = y ~ x, method='lm') +
  # pass a new data for this layer consisting in only 1 row of the initial data
  geom_rect(data =iris[1,], mapping=aes(xmin=0.9, xmax=2, ymin=0, ymax=0.75), alpha=0.2, fill="red", color=NA)
...
### 3. layers with different data and aesthetics
ggplot(iris, aes(Petal.Length, Petal.Width)) +
  geom_point() +
  geom_line() +
  geom_rect(mapping = aes(xmin=2, xmax=3, ymin=0, ymax=4, color="red", fill="green"))

### 4. diferences between mapping data to aesthetics and setting aesthetics values
ggplot(iris, aes(Petal.Length, Petal.Width)) +
  geom_point() +
  geom_line() +
  geom_rect(mapping = aes(xmin=2, xmax=3, ymin=0, ymax=4), color="red", fill="green")

### 5. overwriting aesthetics
ggplot(iris, aes(Petal.Length, Petal.Width, color=Species)) +
  geom_point(color="orange")

### 6. we can remap aesthetics in the desired layer
ggplot(iris, aes(Petal.Length, Petal.Width)) +
  geom_point(aes(shape = Species), alpha=0.2, color="red")

### 7. aesthetics are specific for each layer
ggplot(iris, aes(Petal.Length, Petal.Width)) +
  geom_point(aes(color=Species, size=Petal.Length), alpha=0.4, stroke=3) +
  geom_line()

### 8. aesthetics mapped in ggplot() will be shared between all layers
ggplot(iris, aes(Petal.Length, Petal.Width, color=Species)) +
  geom_point(aes(size=Petal.Length), alpha=0.4, stroke=3) +
  geom_line()

### 9. aesthetics can be specified and mapped in each layer
...
ggplot(iris, aes(x=Petal.Length, y=Petal.Width)) +
  geom_point( ) +
  # geom_label(data = iris[iris$Petal.Length > 6,], mapping=aes(label=Species)) +
  geom_label_repel(data = iris[iris$Petal.Width > 2, ], mapping=aes(label=Species)) +
...
ggplot(iris, aes(x=Species, y=Petal.Length)) +
  geom_boxplot(aes(color=Species, fill=Species), alpha=0.5)+
  geom_jitter()
```
...
ggplot(miris, aes(x=variable, y=value, color=Species, fill=Species)) +
  ...
  geom_jitter(position=position_jitterdodge(jitter.width = .25), size=.75)
```
...
```{r}
ggplot(iris, aes(x=Petal.Length, color=Species)) +
  # position="dodge" to avoid stacked bars
...
# When we precompute the data we have to use `stat="identity"`
...
# We can plot the errorbars through the stats layer without precomputing the data
ggplot(df, aes(x=dose, y=len, fill=supp)) +
  geom_bar(stat="summary", position="dodge") +
  # error bars = standard_deviation (mean_sd)
  stat_summary(fun.data = "mean_sd", geom="errorbar", width=0.25, position=position_dodge(width=0.9)) +
  # error bars = standard_error
  geom_errorbar(stat="summary", position=position_dodge(width=0.9), width=0.25, color="red", linewidth=1.5)
```
...
ggplot(data = faithfuld, aes(waiting, eruptions)) +
...
## Using geom_tile
# Dummy gene expression data
x <- paste0("sample", seq(1,20))
y <- paste0("gene", seq(1,40))
data <- expand.grid(X=x, Y=y)
data$Z <- runif(800, 0, 5)

# Clustering genes
gene_hcl <- hclust(dist(data))

# From long to wide format for clustering samples
wide_data <- spread(data, Y, Z)
rownames(wide_data) <- wide_data$X
wide_data <- wide_data[,-1]

sample_hcl <- hclust(dist(wide_data))

# Give specific order to data
data$X <- factor(data$X, levels=unique(data$X)[sample_hcl$order])
data$Y <- factor(data$Y, levels=unique(data$Y)[gene_hcl$order])


heatmap <- ggplot(data, aes(X, Y, fill= Z)) +
              scale_y_discrete(position = "right") +
              geom_tile() + 
              theme(axis.text.x = element_text(angle=89, vjust=.4, hjust=.5),
                    axis.title = element_blank())

samples_dendro <- ggdendrogram(sample_hcl) + theme_void()
genes_dendro <- ggdendrogram(gene_hcl) + theme_void() +
                  scale_y_reverse() + coord_flip()
design <- "#1
           23"
samples_dendro + genes_dendro + heatmap + plot_layout(design = design, 
                                                 widths = c(2,8),
                                                 heights = c(2,8))

## Using ggheatmap
twide_data <- t(wide_data)
row_metaData <- data.frame(exprtype=sample(c("Up","Down"),40,replace = T),
                           genetype=sample(c("Metabolism","Immune","None"),
                                           40, replace = T))
rownames(row_metaData) <- rownames(twide_data)
col_metaData <- data.frame(tissue=sample(c("Normal","Tumor"),20,replace = T),
                           risklevel=sample(c("High","Low"),20,replace = T))
rownames(col_metaData) <- colnames(twide_data)
...
text_rows <- sample(rownames(twide_data),3)

ggheatmap(twide_data, cluster_rows = T, cluster_cols = T, scale = "col",
              text_show_rows = text_rows,
              cluster_num = c(3,3),
              # Showing the dendograms causes error
...
`geom_bar` uses the default values from stat_count to generate the plot we can access the statistics calculated by stat_count using `after_stat()`
...
`facet_grid` draws 1 plot on a canvas.
...
We need to pass a list of ggplot objects (or 'grobs') as input (A graphical object (“grob”) is a description of a graphical item).
These basic classes provide default behaviour for validating, drawing, and modifying graphical objects.
...
plot_layout <- rbind(c(1,1,3),
                     c(2,2,3))
grid.arrange(grobs = list(p1, p2, legend),
...
library(ggpubr)
ggarrange(p1, legend, p3, p2, ncol = 2, nrow = 2, labels = c("a)", "","b)"))
ggarrange(p1, p3, ncol=1, common.legend = TRUE, legend="right")
```

```{r}
# Using patchwork
p1L <- ggplot(miris, aes(x=value, color=Species)) +
        geom_density() +
        labs(tag = "A")
p3L <- ggplot(iris,aes(x=Petal.Length,y=Petal.Width, color=Species)) +
        geom_point() + theme_minimal()
legend <- get_only_legend(p1L)
p1 <- p1L + theme(legend.position = "none")
p3 <- p3L + theme(legend.position = "none")

# Examples 1,2 & 3
p1 + p3 & plot_annotation(tag_levels = '1')
...
design <- "1112
           3332"
p1 + legend + p3 + plot_layout(design = design)

# Examples 5 & 6
# Collect all legends together
p1L / p3L
p1L / p3L + plot_layout(guides = 'collect')
```
...
The comands follow the pattern `scale_<aesthetic>_<mehtod>()` (`scale_fill_distiller()` - to provide a ColorBrewer palette; `scale_color_viridis()` - to use Viridis).
...
ggplot(iris, aes(x=Sepal.Length, y=Petal.Length, color=Species)) +
  ...
  labs(title="Sepal length vs Petal length",
       subtitle="Diferences between species")+
  theme(panel.background = element_blank(),
        ...
        panel.grid.major = element_line(linewidth=.2, color="grey"),
...
The commands for scales follow the pattern scale\_'aesthetic'\_'mehtod'()
...
```{r}
#scale_fill_viridis_b: binary; c:continuos; d:discrete
x <- paste0("sample", seq(1,20))
y <- paste0("gene", seq(1,40))
data <- expand.grid(X=x, Y=y)
data$Z <- runif(800, 0, 5)
...
# Examples of GGplot2 complexity
...
Find plenty of examples here: <https://r-graph-gallery.com/index.html> Some of the following examples require extra libraries This libraries allow you to use new plotting functions similar to ggplot2.
They also follow the Grammar of Graphic (same as ggplot2)
...
if (! "ggVennDiagram" %in% installed.packages()) {
...
ggvenn(x, columns = c("Stage 1", "Stage 2", "Stage4"),
       fill_color = c("#0073C2FF", "#EFC000FF", "#868686FF", "#CD534CFF"),
       stroke_size = 0.5, set_name_size = 4)
ggvenn(x, columns = c("Stage 1", "Stage 2", "Stage 3", "Stage4"),
       fill_color = c("#0073C2FF", "#EFC000FF", "#868686FF", "#CD534CFF"),
       stroke_size = 0.5, set_name_size = 4)
...
otter_cluster <- hclust(d = dist(x = otter_matrix))
otter_dendro <- as.dendrogram(otter_cluster)
...
otter_long$accession <- factor(x = otter_long$accession,
                               levels=otter_scaled$accession[otter_cluster$order],
                               ordered = TRUE)
...
# It opens an external display window
...
nodes <- network %>% activate(nodes) %>% data.frame() %>% mutate(name = as.numeric(Popularity) - 1)
...
Using `radarchart()` from fmsb Input data format is very specific.
Each row must be an entity.
